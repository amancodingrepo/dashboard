  ...
  try {
    ...
    if (!vanna_base_url) {
      ...
      try {
        ...
        const isDbError =
          fallbackError?.code?.startsWith("P") ||
          fallbackError?.message?.toLowerCase().includes("connect") ||
          fallbackError?.message?.toLowerCase().includes("database") ||
          fallbackError?.message?.toLowerCase().includes("prisma");
      ...
      return res.status(200).json({
        error:
          "AI service not configured and no fallback available for this query.",
        answer:
          "The AI service is not available. Try asking: 'Show top vendors', 'Total spend', or 'Overdue invoices'.",
        sql: null,
        results: [],
        success: false,
      });
    ...
    try {
      ...
      const response = {
        answer: vannaData.success
          ? `I found ${vannaResults.length} result${
              vannaResults.length !== 1 ? "s" : ""
            } for your query.`
      ...
      let userMessage =
        "The AI service is taking too long to respond. Please try a simpler query like 'Show top 5 vendors' or 'Total spend'.";
      ...
      if (e.code === "ECONNABORTED" || e.message?.includes("timeout")) {
        ...
        errorMsg = `Vanna AI service error (${e.response.status}): ${
